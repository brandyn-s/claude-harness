...
|------|---------|-------------|---------------|
| `bash-pretooluse-dispatcher.py` | `Bash\|PowerShell` | Runs the seven unconditional Bash hooks in one Python process, in order: `bash-security-guard.py`, `script-content-guard.py`, `destructive-ops-guard.py`, `git-destructive-checkout-guard.py`, `bash-tail-buffering-guard.py`, `zsh-dialect-guard.py`, `poll-loop-nudge.py`. The first exit 2 wins and stops the chain; an `updatedInput` rewrite is what the hooks after it see; `additionalContext`, `permissionDecision` (strictest wins) and `systemMessage` are merged into one JSON object. Writes a fire row per hook so `bin/hook-fire-report.py` still sees each one. PowerShell payloads reach only `destructive-ops-guard.py`, the one hook that was ever matched on it. | Six `run-hook` + `python3` spawns per Bash call measured 210 ms median (`ls -la`, 2026-09-03); one interpreter does the same work in ~50 ms. Nothing is refactored — each hook runs its own `__main__` under `runpy`, so every one of them still works standalone and its own crash policy still applies. |
| `bash-security-guard.py` | `Bash` | Always blocks credential exposure, exfiltration, reverse shells, security-control disablement, and broad destruction; optionally applies delivery, portability, and workflow policy from `bash_policy_tables.py`. Inline `python -c` bodies and python heredocs also get `check_python_source_exfil`, an ast taint walk (credential files, `SECRET`/`TOKEN`/`KEY` env vars, keychain reads → request bodies, `params=`, URLs) that applies the curl policy to Python: auth headers and the OAuth credential grant pass, a secret in a payload to a host outside `SAFE_RE` blocks | One process and one JSON parse. Fresh-laptop loads the catastrophic core; the author profile sets `CLAUDE_BASH_POLICY_PACKS=all`. Writes its own per-decision audit trail to `~/.claude/audit/bash-security-*.jsonl`. |
| `script-content-guard.py` | `Write\|Edit` (via `write-edit-dispatcher.py`) and `Bash` (via `bash-pretooluse-dispatcher.py`) | Applies `bash-security-guard.py`'s catastrophic checks (`CATASTROPHIC_CHECKS`, one shared tuple) to the BODY of a script file: at Write/Edit time when the target is `.sh`/`.bash`/`.zsh`/`.ksh`/`.py` or opens with a shell/python shebang, and at Bash time when the command runs a local script (`zsh x.sh`, `./x.sh`, `source x.sh`, `python3 x.py`), resolving the path against the payload cwd. Shell files are scanned per logical line after comment stripping; Python files for the shell commands they hand to `subprocess`/`os.system`-style calls (captured output is treated like `$(...)`) and, natively, for a credential reaching a request body, `params=` or URL (`check_python_source_exfil` in the Bash guard — the curl policy in Python; corpus calibration in `hooks/manifests/script-content-guard.yaml`). `CLAUDE_SCRIPT_CONTENT_GUARD=advise` downgrades to `additionalContext`; `off` is the mutation control. Fails closed. | On 2026-08-12 the `${VAR:+set}${VAR:-NOT SET}` diagnostic the Bash guard blocks inline was written to `/tmp/claude/verify_probes.sh` with Write and run as `zsh /tmp/claude/verify_probes.sh`; the guard saw a path and two live API keys reached the transcript. The guard's own remedy text ("write the code to a .py FILE and run it") pushed work into that blind spot, so compliance lowered protection. Shipped 2026-09-06 after a corpus replay (`bin/replay-script-content-guard.py`). |
...
| `read-deny-guard.py` | `Read` | Deterministically denies Read-tool access to secret paths: a static default list (`~/.ssh/**`, `~/.aws/**`, `**/.env*`, credential and key files) plus every `Read(...)` rule in `permissions.deny`. Part of the fresh-laptop core. | Compensating control for anthropics/claude-code #88795 / #88770: on 2.1.239 with `defaultMode: auto`, Read-tool deny rules in settings.json were silently not enforced. The sandbox denies the same paths to Bash, so nothing prompts. |
| `rule-size-guard.py` | `Write\|Edit` (via `write-edit-dispatcher.py`) | Enforces the per-file (WARN 35,000 / BLOCK 38,000 chars) and aggregate ambient-rule budgets from `hooks/rule_context_budget.py` and `manifests/ambient-budget.json` when a rule file is written. | Ambient rules load into every session; files over ~40K chars trigger a Claude Code performance warning, and `/distill` used to append incidents without checking size (descopes recurred 2026-05-18/19/21). |
| `home-scratch-guard.py` | `Write\|Edit` (via `write-edit-dispatcher.py`) | Warns (never blocks) when a non-dotfile with a scratch-like suffix is written directly to `$HOME`. | Scratch scripts, run logs and report `.md` files accumulated in the home root instead of `/tmp/claude` or a repo. |
...
| Hook | Matcher | What it does | Why it exists |
|------|---------|-------------|---------------|
| `output-secret-redact.py` | `Bash\|Read\|mcp__.*` | Redacts high-confidence secrets (key-shaped tokens, private keys, cloud-CLI credential output) from tool OUTPUT before the model reads it. Part of the operator layer. | Defence-in-depth below credential-guard and the sandbox `denyRead` globs: a tool's output can surface a secret that never touched a denied path (`env` dumps, `aws sts` output, config files read by a script). |
...
| `compaction-budget.py` | PostCompact, UserPromptSubmit | Counts compactions per session (PostCompact cannot inject, so the count is stored) and from the second one raises a handoff nudge on the next prompt: report evidence and stop if the frame is complete, otherwise write `HANDOFF.md` (working directory + `~/.claude/HANDOFF.md`, which `session-start.py` shows once) and propose a fresh session. Also writes the acceptance ledger's `compaction_count`, audits the `compact_summary` against the ledger (`session_ledger.audit_against_summary`, token containment) and appends the row to `~/.claude/audit/ledger-audit-YYYYMMDD.jsonl`; a REJECTED entry the summary lost is named once on the next prompt (`<ledger-audit>`). `bin/ledger-audit-report.py` rolls the rows up — that report is the test of the hypothesis `session_ledger.py` was built on. Never blocks. | The 2026-08-30 week's three longest arcs (20/24/39 h) each crossed three or more compactions; the 14-day ledger review put 35 of 58 corrective turns in six long sessions. `never-stop-early.md` was deleted 2026-09-03 for fighting the stop contract and nothing then said what to write at a boundary; `rules/session-boundaries.md` says it and this hook raises it. |
| `proceed-gate.py` | UserPromptSubmit | On a bare continuation (`proceed`, `continue`, `do it all`, `fix 1, 2 and 4`, `1,2,3`) injects a DO / NOT / CHECK restatement contract and lets the model proceed; nudges `/frame` once per session when a substantive imperative arrives in a directory with no `INTENT.md` (never for questions); ingests `INTENT.md` Done-when / Non-goals / Constraints bullets into `session_ledger.py` once per content hash. Never blocks. | A third of the 2026-08-30 week's prompts were bare continuations and done-criteria emerged over hours; the acceptance ledger `session-start.py` rehydrates after every compaction had a reader and no writer -- this is its first producer. |
