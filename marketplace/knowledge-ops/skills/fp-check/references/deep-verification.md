...
For each bug (Bug #N), track tasks with the dependency structure below. Maintain task IDs and dependency wiring in the conversation transcript (this skill's `allowed-tools` does not include TaskCreate/TaskUpdate — use plain text task lists with explicit `blocked by` annotations).
...
- **Parallel sub-phases**: Launch independent sub-phases concurrently using the skill's agents (defined in ~/.claude/agents/). Collect all results before proceeding to the next dependency gate.
...
Spawn these agents via the `Agent` tool (declared in SKILL.md `allowed-tools` and `manifest.yaml requires_tools`) for their respective phases. Pass the bug description and any prior phase results as context.
