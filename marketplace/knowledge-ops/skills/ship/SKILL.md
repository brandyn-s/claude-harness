...
python3 "${CLAUDE_PLUGIN_ROOT}/bin/pr-merge-verified.py" "$PR_NUMBER" --repo "$REPO_SLUG"

# /ship --queue-only: exits 0 after a durable arm/queue entry (or MERGED).
python3 "${CLAUDE_PLUGIN_ROOT}/bin/pr-merge-verified.py" "$PR_NUMBER" --repo "$REPO_SLUG" --queue-only
```
...
Read `${CLAUDE_PLUGIN_ROOT}/skills/_shared/linear-routing.md`. Look up repo name.
