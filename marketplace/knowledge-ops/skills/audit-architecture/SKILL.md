...
**MCP connectivity:** Probe only the **connectible** class — locally-registered stdio servers and known-authenticated HTTP servers — by calling one lightweight tool each (ping map in `references/probe-targets.md`); flag failures with the specific error message. Do NOT probe **gateway / OAuth-pending** servers (those whose only exposed tools are `authenticate`/bootstrap — typically remote OAuth gateways): probing them starts an OAuth flow or bills an expensive call. Classify each such server `AUTH-PENDING` (registered, not probed) — this is NOT a connectivity gap. Report R1 as `probed OK / probed FAIL / AUTH-PENDING (not probed)`, never implying full 1-per-server coverage when the gateway class was skipped.
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/audit-architecture/references/discovery.py > discovery.json
...
Start from `discovery.json`'s `coverage` map (topic file, PreToolUse, CLAUDE.md mention). Server-to-topic aliases are explicit, never fuzzy: the repo ships an EMPTY alias map, and environment-specific aliases come from the optional file `$CLAUDE_CONFIG_DIR/audit-architecture/aliases.json` (else `~/.claude/audit-architecture/aliases.json`) — a flat `{"server-name": "topic-alias"}` object that `discovery.json` echoes under `alias_file`/`aliases`. A missing file means no aliases; a malformed one exits 2. When a server's topic file has a different name, add an alias there rather than loosening the matcher. Apply the host profile in `audit-context.md` before flagging: C1/C5 may be N/A by design. For each MCP server discovered in Phase 1, the dimensions are:
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/audit-architecture/references/skill_quality_audit.py >quality.json 2>quality_report.txt
...
1. **Probe target accuracy**: Run `python3 ${CLAUDE_PLUGIN_ROOT}/skills/audit-architecture/references/gen_probe_targets.py --check` — the current-host block of `probe-targets.md` is GENERATED from live config between markers; exit 1 means drift (fix with `--write`, which is idempotent — never hand-edit inside the markers). The server list exists only inside the markers — the repo ships no inventory — and the prose outside them is host-independent guidance. Zero registered servers renders a one-line block and is not a finding.
...
4. **Coverage matrix fallback**: For servers outside the generated block's verified-cheap table, use ToolSearch to load one read-only list/get tool as the connectivity check (the fallback paragraph in `probe-targets.md`).
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/audit-architecture/references/doc_accuracy_audit.py >doc_accuracy.json 2>doc_report.txt
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/audit-skill-oracle.py reverify \
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/audit-skill-oracle.py refresh-tracker \
