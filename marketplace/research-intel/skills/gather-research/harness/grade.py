#!/usr/bin/env python3
"""Pure, key-free scoring for the gather-research live-arm efficacy harness.
...
deterministic instrument that:
  ...
  - tests/test_gather_research_efficacy.py unit-tests on a tiny synthetic
...
def decide_verdict(with_skill: dict, baseline: dict, primary_metric: str,
    ...
    fix   : a specific sub-metric underperforms badly (refutation or fabrication
            resistance below baseline) even if the primary metric is flat.
    """
    def mean(d, k):
    ...
    # sub-metric regression check
    regressions = []
    for k in ("refutation_recall", "fabrication_resistance", "grounding_precision"):
        wk, bk = mean(with_skill, k), mean(baseline, k)
        if wk is not None and bk is not None and wk + 1e-9 < bk:
