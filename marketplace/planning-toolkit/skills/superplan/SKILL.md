...
PHASE 0 — Reality Check
  ✓ /superplan exists at ${CLAUDE_PLUGIN_ROOT}/skills/superplan/SKILL.md (667 lines)
...
Pattern from `OthmanAdi/planning-with-files`'s `/plan-attest`. The mtime-keyed cache avoids re-hashing on every turn (~99% I/O savings when the plan is unchanged) — see `${CLAUDE_PLUGIN_ROOT}/skills/supergoal/references/verification-hook.md` Step 1 for the full procedure.
