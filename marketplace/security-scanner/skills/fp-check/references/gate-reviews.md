...
| **5. Math Bounds** | Mathematical analysis confirms vulnerable condition is possible | Bounds argument shows condition is possible | Math proves validation prevents it |
...
- **TRUE POSITIVE**: All gate reviews pass → `BUG #N TRUE POSITIVE — [brief vulnerability description]` with `confidence_score: HIGH|MEDIUM|LOW` and `exploitability_rating: EASY|MEDIUM|HARD|NOT_EXPLOITABLE`
- **FALSE POSITIVE**: Any gate review fails → `BUG #N FALSE POSITIVE — [brief reason for rejection]` with `confidence_score: HIGH|MEDIUM|LOW`
