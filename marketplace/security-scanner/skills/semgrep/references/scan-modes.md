...
for f in "$OUTPUT_DIR/raw"/*-*.json; do
  [[ "$f" == *-triage.json || "$f" == *-important.json ]] && continue  # Skip triage files (vestigial from retired triage mechanism; retained as defensive guard)
  jq '{
    results: [.results[] |
      ((.extra.metadata.category // "security") | ascii_downcase) as $cat |
      ((.extra.metadata.confidence // "HIGH") | ascii_upcase) as $conf |
...
- **Run all**: *(empty)*
- **Important only**: `--severity ERROR --severity WARNING`
