...
- **Important only**: Add `--severity ERROR --severity WARNING` to every `semgrep` command
...
- **Important-only mode:** Run the post-filter first (creates `*-important.json` in `raw/`), then run the merge script. Raw SARIF files are unaffected by the JSON post-filter, so the merge operates on the unfiltered SARIF. SARIF-level filtering is **not yet implemented** (scan-modes.md describes a JSON post-filter against `.results[]`, which is the Semgrep JSON shape; a SARIF-targeted filter against `runs[].results[]` would need to be written). If important-only SARIF is required, filter the resulting `results.sarif` manually with a jq query like `'.runs[].results |= map(select(.properties.severity == "ERROR" or .level == "error"))'`.

**Verify merged SARIF is valid:**

```bash
python3 -c "import json; d=json.load(open('$OUTPUT_DIR/results/results.sarif', encoding='utf-8')); print(f'{sum(len(r.get(\"results\",[]))for r in d.get(\"runs\",[]))} findings in merged SARIF')"
```
