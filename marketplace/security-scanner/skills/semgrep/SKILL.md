---
name: semgrep
description: "Scan code with Semgrep (SAST) using parallel subagents; auto-detects Pro for taint analysis."
when_to_use: 'Use when scanning code for vulnerabilities or running static analysis. Trigger phrases: "run semgrep", "SAST scan", "static analysis", "security scan", "code scan". Do NOT use for CodeQL (use /codeql) or custom rule writing (use /semgrep-rule-creator). Runs Semgrep with parallel subagents in "run all" (full ruleset) or "important only" (high-confidence) modes. Auto-detects Semgrep Pro for cross-file taint analysis.'
allowed-tools: Agent AskUserQuestion Bash Glob Read Task
effort: high
argument-hint: "[repo-path or directory to scan]"
metadata:
  author: example-security-engineering
  version: "1.0"
compatibility:
  # Requires semgrep CLI. Optional: Semgrep Pro for cross-file taint analysis.
  requires:
    - cli: semgrep
...
- Porting existing rules to other languages → Use `/semgrep-rule-creator` skill
...
```bash
semgrep --pro --validate --metrics=off --config p/default 2>/dev/null && echo "Pro available" || echo "OSS only"
```

**Limitations:** OSS mode cannot track data flow across files. Pro mode is slower per ruleset (cross-file analysis is more expensive than single-file scanning), but parallel rulesets compensate.
...
1. **Pre-filter**: `--severity ERROR --severity WARNING` (CLI flag — semgrep's `--severity` accepts only INFO/WARNING/ERROR, so medium-and-up maps to WARNING+ERROR; the metadata post-filter below does the MEDIUM/HIGH narrowing)
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/semgrep/scripts/merge_sarif.py $OUTPUT_DIR/raw $OUTPUT_DIR/results/results.sarif
```
...
- [ ] Every scan and validation `semgrep` command used `--metrics=off` (version checks like `semgrep --version` are exempt, per Essential Principle 1)
...
- [ ] Important-only mode: JSON post-filter (`*-important.json`) applied before merge; SARIF-level filtering is not yet implemented (merge operates on unfiltered SARIF); unfiltered results preserved in `raw/`
...
User says: "/semgrep" in a Python/TypeScript monorepo
Actions: Auto-detect languages, check Semgrep Pro availability, present scan mode options, get user approval at hard gate, dispatch parallel scanners per language, merge SARIF results.
Result: Consolidated SARIF with findings from all languages, presented as prioritized table.

**Example 2: Targeted rule scan**
User says: "/semgrep important-only on mcp-servers"
Actions: Select important-only ruleset (security-focused subset), run against specified repo, present high-confidence findings only.
Result: Focused security findings without noise from style/quality rules.
