...
- Tool restriction via `coreTools` allowlist in settings JSON (e.g., `["run_shell_command(echo)"]`)
...
| `coreTools` containing `run_shell_command(echo)` | Enables subshell expansion bypass -- confirmed RCE vector (Vector F) |
...
**Restrict tool lists:** Remove `run_shell_command(echo)` and other expandable commands from `coreTools`. Use specific non-shell tools only:
...
|-----------------|-------------------|--------------|------------|-------------------|
| Restrict shell access | `--allowedTools "Bash(specific:*)"` | `sandbox: workspace-write` | Remove expandable commands from `coreTools` | N/A (no shell) |
