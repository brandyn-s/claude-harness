...
description: "Audit GitHub Actions workflows for AI-agent security holes (attacker input reaching Claude Code Action or Codex)."
when_to_use: "Use when reviewing workflows with Claude Code Action, Gemini CLI, OpenAI Codex, or GitHub AI Inference. Trigger phrases: \"audit actions\", \"review workflow security\", \"CI agent vulnerabilities\". Detects attack vectors where attacker-controlled input reaches AI agents in CI/CD: env var intermediary, direct expression injection, dangerous sandbox configs, wildcard allowlists. Do NOT use for general code scanning (use /semgrep or /codeql)."
allowed-tools: Read Grep Glob Bash AskUserQuestion
effort: high
argument-hint: "[repo-path or PR-URL]"
metadata:
  author: example-security-engineering
  version: "1.0"
compatibility:
  # Requires gh CLI for workflow file enumeration across repos.
  requires:
    - cli: gh
...
**Example 1: Full workflow audit**
User says: "/agentic-actions-auditor" in a repo with `.github/workflows/`
Actions: Enumerate all workflow files, identify AI agent invocations (Claude Code Action, Gemini CLI), trace data flow from triggers through env vars to agent inputs, generate findings report with severity ratings.
Result: Structured findings report with 0-N vulnerabilities, each citing specific file:line and data flow path.

**Example 2: Targeted PR review**
User says: "audit the new CI workflow in PR #42 for agentic security issues"
Actions: Read the PR diff, focus on new/changed workflow steps invoking AI agents, check for expression injection via `${{ }}`, verify sandbox configuration, assess allowedTools scope.
Result: Security assessment of the PR's agent integration with PASS/FAIL per vector.
## Success Criteria

- Every finding cites a specific workflow file path, line number, and data flow trace
- No findings based on file names or grep matches alone — each requires reading the actual workflow YAML
- Attack vectors assessed include: expression injection, env var intermediaries, untrusted trigger contexts, wildcard allowlists, and sandbox escape
- False positive rate < 20% when verified against actual exploitability
- Report includes severity judgment (High/Medium/Low/Info) with rationale for each finding
