...
```bash
find . -type f \( \
  -name '*.py' -o -name '*.js' -o -name '*.ts' -o -name '*.go' -o -name '*.rb' \
...
# Also detect GitHub Actions workflows — if present, we MUST build a separate
# `actions` database (see SKILL.md Principle 2). Single-language audits silently
# miss CI-specific findings (`unpinned-tag`, `missing-workflow-permissions`,
# `untrusted-checkout`).
HAS_ACTIONS=false
if [ -d .github/workflows ] && [ -n "$(find .github/workflows -maxdepth 1 -type f \( -name '*.yml' -o -name '*.yaml' \) 2>/dev/null)" ]; then
  HAS_ACTIONS=true
  echo "Detected .github/workflows/ — actions database will be built in Step 2 (see Method 5)"
fi
```
...
| GitHub Actions | `actions` | Interpreted (workflow YAML) |
...
# Array form — works in bash AND zsh (zsh does not word-split an unquoted $CMD string)
...
|--------------|-----------|---------|
| Make | `Makefile` | `make clean && make -j$(getconf _NPROCESSORS_ONLN)` |
...
### Additional Step: Build `actions` Database (when `.github/workflows/` exists)

> **Operationalizes SKILL.md Principle 2.** When `HAS_ACTIONS=true` from Step 1a, build a **second** database for the `actions` language alongside the primary language database. The actions suite catches CI-specific findings (`unpinned-tag`, `missing-workflow-permissions`, `untrusted-checkout`) that source-language suites do not find.

Run this step **after** the primary language database succeeds (Step 4 reaches GOOD quality). Do not skip — single-language audits silently miss findings GitHub's default-setup catches via its actions analysis.

```bash
if [ "$HAS_ACTIONS" = "true" ]; then
  ACTIONS_DB="$OUTPUT_DIR/codeql-actions.db"
  log_step "ADDITIONAL: Build actions database for .github/workflows/"
  CMD=(codeql database create "$ACTIONS_DB" --language=actions --source-root=. --overwrite)
  log_cmd "${CMD[*]}"
  "${CMD[@]}" 2>&1 | tee -a "$LOG_FILE"

  log_result "Built actions database alongside $CODEQL_LANG database. Both will be analyzed."
fi
```

The actions database is analyzed in the run-analysis workflow using `actions-security-and-quality.qls` (see `references/ruleset-catalog.md` and `workflows/run-analysis.md` Step 4). If the actions database build fails, log the failure but continue with the primary database; do not abort the workflow.
