...
find . -type f \( -name 'qlpack.yml' -o -name 'codeql-pack.yml' \) \
  -not -path '*/static_analysis_codeql_*/*' \
  -not -path '*/codeql*.db/*' \
...
# 2. Standalone data extension files — must contain BOTH `extensions:` and `addsTo:`
# (the data-extension format requires `addsTo:`, so a bare `extensions:` key from an
# unrelated YAML — e.g., a GitHub Actions workflow — is correctly excluded).
find . -type f -name '*.yml' \
  -not -path '*/static_analysis_codeql_*/*' \
  -not -path '*/codeql*.db/*' \
  -not -path '*/\.*' 2>/dev/null | while read -r f; do
  if grep -q '^extensions:' "$f" && grep -q 'addsTo:' "$f"; then
    echo "$f"
  fi
done | head -20
...
```bash
BASELINE=$(python3 -c "import json; print(sum(len(r.get('results',[])) for r in json.load(open('$DIAG_DIR/baseline.sarif', encoding='utf-8')).get('runs',[])))")
WITH_EXT=$(python3 -c "import json; print(sum(len(r.get('results',[])) for r in json.load(open('$DIAG_DIR/with-extensions.sarif', encoding='utf-8')).get('runs',[])))")
