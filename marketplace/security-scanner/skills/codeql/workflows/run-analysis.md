...
**Scope the `.yml` recursive search** to `$OUTPUT_DIR/extensions/` (where create-data-extensions writes them) plus the project root excluding `.git/`, `node_modules/`, and any directory containing virtualenv/installed-package YAML. Without scoping, a recursive search would match unrelated config files (e.g., GitHub Actions workflows, Helm charts, OpenAPI specs) that happen to contain an `extensions:` key. The reliable filter is: file must contain BOTH `extensions:` AND `addsTo:` keys (the data-extension format requires `addsTo`).
...
#### Also run actions analysis (when `.github/workflows/` exists)

> **Operationalizes SKILL.md Principle 2.** When build-database created an `actions` database (see `find $OUTPUT_DIR -maxdepth 2 -name 'codeql-database.yml'` returning multiple results), also run the actions suite. Single-language audits silently miss CI-specific findings.

```bash
ACTIONS_DB="$OUTPUT_DIR/codeql-actions.db"
if [ -f "$ACTIONS_DB/codeql-database.yml" ]; then
  echo "Running actions analysis against $ACTIONS_DB"
  # Use the built-in actions suite (no custom .qls needed — actions has no third-party packs)
  codeql database analyze "$ACTIONS_DB" \
    --format=sarif-latest \
    --output="$RAW_DIR/results-actions.sarif" \
    --threads=0 \
    -- codeql/actions-queries:codeql-suites/actions-security-and-quality.qls

  # Merge actions findings into the main raw SARIF for downstream processing
  if [ -f "$RAW_DIR/results-actions.sarif" ]; then
    jq -s '.[0].runs += .[1].runs | .[0]' \
      "$RAW_DIR/results.sarif" "$RAW_DIR/results-actions.sarif" \
      > "$RAW_DIR/results-merged.sarif"
    mv "$RAW_DIR/results-merged.sarif" "$RAW_DIR/results.sarif"
  fi
fi
```
