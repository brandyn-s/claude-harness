...
description: "Write custom Semgrep rules for security vulnerabilities and bug patterns."
when_to_use: "Creates custom Semgrep rules for detecting security vulnerabilities, bug patterns, and code patterns. Use when writing Semgrep rules or building custom static analysis detections. Do NOT use for running existing rules (use /semgrep), CodeQL queries (use /codeql), or SARIF output analysis (use /sarif-parsing)."
allowed-tools: Bash Read Write Edit Glob Grep WebFetch AskUserQuestion
effort: high
argument-hint: "[vulnerability-pattern to detect]"
metadata:
...
- General static analysis without custom rules (use `/semgrep`)
...
- [ ] Step 7: Final Run
- [ ] Step 8: Validate FP rate against a real codebase (run the rule against a representative repo; inspect findings; record FP count and adjust rule until acceptable)
```
...
User says: "/semgrep-rule-creator detect hardcoded API keys in Python"
Actions: Understand the pattern, write a Semgrep rule with metavariables and pattern-matching, create test cases (positive and negative), validate the rule runs cleanly.
Result: Production-quality .yaml rule with test file and documentation.

**Example 2: Framework-specific rule**
User says: "write a Semgrep rule for FastMCP handlers missing authentication"
Actions: Study FastMCP handler patterns in the codebase, identify the authentication decorator/check, write a rule that matches handlers WITHOUT the check.
Result: Custom rule targeting framework-specific patterns with low false-positive rate.
## Success Criteria

- Rule includes both positive tests (should match) and negative tests (should not match)
- Test-first approach: tests written before the rule, then rule iterated until tests pass
- Rule uses appropriate Semgrep features (metavariables, pattern-not, pattern-inside) for precision
- False positive rate validated against real codebase before shipping
