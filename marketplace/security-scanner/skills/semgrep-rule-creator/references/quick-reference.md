...
**Forbidden annotations**: `todoruleid: <rule-id>` and `todook: <rule-id>` (used elsewhere for future rule improvements) are not allowed in this skill's tests. Use only `ruleid:` and `ok:`.
