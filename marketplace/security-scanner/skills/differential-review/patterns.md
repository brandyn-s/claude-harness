...
**Specialized Pattern Resources** (external — not present in this
repository; consult the upstream source by name when relevant):

**Domain-Specific** (from upstream audit corpora):
...
- Other domains under `domain-specific-audits/skills/` in the upstream repo

**Solidity-Specific** (external repos by name, not local paths):
- `not-so-smart-contracts` — Automated Solidity vulnerability detectors (Trail of Bits)
...
These complement the generic patterns below. They are not present
on disk here; pull from the upstream sources when you need them.
...
```bash
git diff <range> | grep -E "^[+-][^+-]" | grep -E "[][:alnum:]_)] *[-+*/] *[[:alnum:]_(]|[-+*/]=|\+\+|--"
```
