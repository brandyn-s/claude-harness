...
Checkout the baseline commit and manually inventory the baseline using Read,
Grep, and Glob — the goal is to capture system-wide context BEFORE looking at
the diff, so you can detect when changes violate baseline invariants.
...
**Manual baseline analysis checklist** (read and Grep on the baseline tree):
...
**Prerequisite check** — the `gh` CLI is required only when the target is a
GitHub PR number. For commit ranges or branches, plain `git` suffices.

```bash
# Verify gh is available before using `gh pr view`
command -v gh >/dev/null || echo "gh not installed — use commit-range or branch target instead"
```

If `gh` is missing and the target is a PR, ask the user for the base..head
commit range (or `origin/main..HEAD`) and proceed with the `git`-only path.

**Extract changes:**
```bash
# For commit range or branch (no external dependency)
git diff <base>..<head> --stat
git log <base>..<head> --oneline

# For PR (requires gh CLI — see prerequisite check above)
...
- **MEDIUM**: 20-199 files → Focused analysis (1-hop deps)
...
For each HIGH RISK changed function, perform a line-by-line analysis using
Read, Grep, and manual code tracing to answer the following:
