---
name: sharp-edges
description: "Identify error-prone APIs, footgun configs, and insecure-by-default designs."
when_to_use: "Identifies error-prone APIs, dangerous configurations, and footgun designs that enable security mistakes. Use when reviewing API designs, configuration schemas, cryptographic library ergonomics, or evaluating whether code follows 'secure by default' and 'pit of success' principles. Triggers: sharp-edges, footgun, misuse-resistant, secure defaults, API usability, dangerous configuration. Do NOT use for security vulnerability scanning (use /semgrep or /codeql), code review (use /differential-review), or runtime debugging (use /systematic-debugging)."
allowed-tools: Read Grep Glob
effort: high
argument-hint: "[codebase-path or feature-area]"
metadata:
...
1. **Reproduce the misuse**: Provide a minimal code snippet demonstrating the footgun (embedded as PoC in the report, not executed)
...
4. **Test mitigations**: Analyze whether the API can be used safely with reasonable effort (code examples or reasoning in the report; testing is analytic, not executed)
...
**Example 1: Crypto API review**
User says: "/sharp-edges" on a Rust service with TLS configuration
Actions: Identify crypto-related APIs (rustls config, certificate handling), check for algorithm selection footguns (allowing weak ciphers by default), verify configuration cliff patterns (one missing field changes security posture), assess silent failure modes.
Result: Sharp edges report with severity classification per API surface.

**Example 2: Configuration review**
User says: "/sharp-edges review our MCP server configs"
Actions: Analyze configuration patterns for dangerous defaults, boolean flags with non-obvious security implications, stringly-typed security settings, and silent degradation paths.
Result: Configuration sharp edges ranked by likelihood of developer misuse.
## Success Criteria

- Six sharp-edge categories assessed: algorithm selection footguns, dangerous defaults, primitive vs semantic APIs, configuration cliffs, silent failures, stringly-typed security
- Each finding includes: the sharp edge, how a developer would reasonably misuse it, and the pit-of-success alternative
- Language-specific patterns checked using the appropriate references/ file
- Severity reflects likelihood of misuse, not just severity of consequence
