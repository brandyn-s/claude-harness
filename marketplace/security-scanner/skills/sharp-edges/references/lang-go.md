...
**Note**: Per-iteration loop-variable scoping became the default in Go 1.22 — no flag needed. `GOEXPERIMENT=loopvar` was the Go 1.21 preview.
...
data, _ := os.ReadFile(filename)  // Error ignored! (ioutil.ReadFile was deprecated in Go 1.16 — use os.ReadFile)
