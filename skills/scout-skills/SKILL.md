...
if echo "$content" | grep -q "Not Found"; then
  ...
  echo "$content" | tr -d '\n' | python3 ~/.claude/skills/scout-skills/scripts/decode_contents.py
fi
```
...
```bash
echo "$content" | tr -d '\n' | python3 ~/.claude/skills/scout-skills/scripts/decode_contents.py > ${TMPDIR:-/tmp}/scout-skills/scan-fetch-<repo>-<skill>.md
```
...
```bash
python3 ~/.claude/skills/scout-skills/scripts/produce_card.py \
