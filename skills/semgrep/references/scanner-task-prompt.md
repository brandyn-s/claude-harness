...
- If scan mode is **important-only**, add `--severity ERROR --severity WARNING` to every command
...
| `[SEVERITY_FLAGS]` | Severity pre-filter flags | *(empty)* for run-all, `--severity ERROR --severity WARNING` for important-only |
