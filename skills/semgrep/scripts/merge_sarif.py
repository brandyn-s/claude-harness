...
Usage:
    python3 merge_sarif.py RAW_DIR OUTPUT_FILE
...
def merge_sarif_pure_python(sarif_files: list[Path]) -> dict:
    ...
    for sarif_file in sorted(sarif_files):
        ...
        for run in data.get("runs", []):
            if tool_info is None and run.get("tool"):
                # Deep-copy so we don't share a reference with the parsed
                # input data — line 142 below writes through this dict
                # to attach merged rules, which would mutate the caller's
                # SARIF if it were the original object.
                tool_info = copy.deepcopy(run["tool"])
    ...
    if skipped_files:
        ...
        # If every file failed to parse, the merge produced no signal —
        # don't write an empty-runs SARIF and pretend it succeeded.
        if sarif_files and len(skipped_files) == len(sarif_files):
            print(
                f"ERROR: All {len(sarif_files)} SARIF files were unparseable; "
                f"refusing to write empty merge.",
                file=sys.stderr,
            )
            raise RuntimeError("all_sarif_files_unparseable")

    return merged


USAGE = (
    "usage: merge_sarif.py RAW_DIR OUTPUT_FILE\n"
    "  Merge all *.sarif files in RAW_DIR into OUTPUT_FILE.\n"
    "  Uses SARIF Multitool when available; falls back to pure Python.\n"
