...
### Self-Documenting Danger Flags

When a security-disabling flag is genuinely unavoidable (e.g. an opt-out for a
sandboxed-execution path), make the flag's **name** carry the warning, so enabling
it is self-documenting and socially costly — not a bland `unsafe = true`.

```python
# BAD: trivially flipped without reckoning with the consequences
UNSAFE = True
SKIP_SANDBOX = True

# GOOD: the name forces the operator to assert the risk, is greppable, and is
# embarrassing to enable casually.
# (Nicholas Carlini, yet-another-applied-llm-benchmark/docker_controller.py)
I_HAVE_BLIND_FAITH_IN_LLMS_AND_AM_OKAY_WITH_THEM_BRICKING_MY_MACHINE = False
```

**Principle**: the only thing between an operator and a dangerous mode should be a
name they cannot enable without acknowledging exactly what they are turning off.
Pairs with the checklist item "No security-disabling booleans: or require
confirmation/separate config."
