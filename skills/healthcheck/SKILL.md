...
> `python3 $HOME/.claude/skills/healthcheck/references/_check_all.py` (add
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_freshness.py
```
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_hooks_aux.py
```
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_config.py
```
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_skills.py
```
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_memory.py
```
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/check_paths.py
```
...
```bash
python3 $HOME/.claude/bin/staged-spec-staleness.py
```
...
```bash
python3 $HOME/.claude/skills/audit-architecture/references/doc_accuracy_audit.py 2>/dev/null
```
...
| Skills | `~/.claude/skills/*/SKILL.md` directories | ARCHITECTURE.md skill inventory table |
...
`~/.claude/skills/`. Flag deprecated references.
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_routing.py
```
...
missing from `~/.claude/skills/`), dead agent refs (agent `.md` missing from
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_targets.py
```
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_orphans.py
```
...
`~/.claude/hooks/` and `~/.claude/scripts/` — so a skill referencing its own
...
Source of truth: `$HOME/.claude/skills/*/SKILL.md`. Publish target:
`$HOME/.claude/marketplace/<plugin>/skills/<name>/SKILL.md`, assembled by
`$HOME/.claude/scripts/build-marketplace.py` from the `PLUGINS` list.
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_manifest.py
```
...
- `"Indexes: FAIL — {K} issues"` + per-issue line, plus remediation pointers:
  - `python3 $HOME/.claude/scripts/cleanup-indexes.py` (plan aborted-index + duplicate-hash cleanup; `--execute` to delete)
...
```bash
python3 $HOME/.claude/skills/healthcheck/references/_check_launchd.py
```
