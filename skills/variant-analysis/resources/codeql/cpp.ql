...
module VariantConfig implements DataFlow::ConfigSig {
  predicate isSource(DataFlow::Node source) {
    ...
    exists(FunctionCall fc |
      fc.getTarget().getName() in ["gets", "scanf", "fscanf", "sscanf", "getline", "getchar", "fgetc"] and
      source.asExpr() = fc
    )
    or
    // Network input
  ...
  predicate isBarrier(DataFlow::Node node) {
    // Input validation
    // NOTE: strlen/strnlen/isalpha/isdigit/isalnum removed from barriers —
    // these measure or classify data, they do not transform it; marking
    // them as sanitizers produces false negatives (taint flows through
    // them downstream regardless). Same removal for snprintf which is
    // declared as a format-string sink at line 60-63 above — listing it
    // here as a barrier made every snprintf call self-sanitize, so the
    // sink-side declaration never fired.
    exists(FunctionCall fc |
      fc.getTarget().getName() in ["strncpy", "strncat"] and
      node.asExpr() = fc
    )
    or
    // Sanitization
