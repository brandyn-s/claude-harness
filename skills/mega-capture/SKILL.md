...
Breadth ACROSS themes, consolidation WITHIN a theme: every distinct capturable theme reaches SOME KB
entry (none dropped for living in the compacted-away head), repeated mentions of one theme merge into
one entry, and an oversized theme SPLITS rather than truncates. THEME-DROPPING is the failure mode;
rationale and the non-goals list: `references/coverage-principle.md`.
