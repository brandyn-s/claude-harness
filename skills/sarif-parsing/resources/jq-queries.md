...
# By file path (contains) — `[]?` skips results without a locations array
...
# Filter and create new SARIF with only errors (|= filters each run's own
# results; plain = would copy the cross-run union of errors into every run)
jq '.runs[].results |= map(select(.level == "error"))' results.sarif > errors-only.sarif
