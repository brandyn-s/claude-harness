...
The `--threat-model` flag can be repeated. Each invocation adds (or removes with `!` prefix) a threat model group. By default CodeQL enables the `default` group, which contains `remote`. The `all` group enables everything, and `!<name>` disables a specific model or group.

**Disabling the default (remote) group is rare and intentionally awkward.** If you genuinely need to analyze without remote-source taint flow (e.g., a CLI tool where only local input matters and remote findings would be pure noise), pair `!default` with at least one positive group so CodeQL has *something* to taint:

```bash
codeql database analyze codeql.db \
  --threat-model local --threat-model '!default' \
  -- results/suite.qls
```

The exact behavior of `--threat-model '!default'` should be verified once on a machine with `codeql` installed by running the command above against a small project with both remote and local sources, then confirming the resulting SARIF contains only `local`-tainted flows. The CodeQL CLI documentation describes group names and the `!`-prefix syntax, but skill maintainers should still confirm against a real installation before relying on this in production audits.
