...
Generate this file as `important-only.qls` in the raw directory (`$OUTPUT_DIR/raw/`) before running analysis. The unfiltered suite output lives in `raw/`; the filtered final results are written to `results/` by the post-analysis filter:
