---
name: codeql
description: "Deep dataflow and taint-tracking security analysis with CodeQL."
when_to_use: "Scans a codebase for security vulnerabilities using CodeQL's interprocedural data flow and taint tracking analysis. Use when a codebase needs deep dataflow or taint-tracking security analysis, or when a CodeQL database must be built or queried. Triggers on \"run codeql\", \"codeql scan\", \"codeql analysis\", \"build codeql database\", or \"find vulnerabilities with codeql\". Supports \"run all\" (security-and-quality suite) and \"important only\" (high-precision security findings) scan modes. Also handles creating data extension models and processing CodeQL SARIF output. Do NOT use for pattern-based scanning (use /semgrep), false positive triage (use /fp-check), or SARIF file parsing (use /sarif-parsing)."
allowed-tools: AskUserQuestion Bash Edit Glob Grep Read Write
effort: high
argument-hint: "[repo-path or language to scan]"
metadata:
  author: example-security-engineering
  version: "1.0"
compatibility:
  # Requires codeql CLI binary installed locally.
  requires:
    - cli: codeql
...
Supported languages: Python, JavaScript/TypeScript, Go, Java/Kotlin, C/C++, C#, Ruby, Rust, Swift, and GitHub Actions workflows (`actions`).
...
2. **Audit ALL detected languages, not just the primary one.** When the target repo has `.github/workflows/`, also build an `actions` database and run `actions-security-and-quality.qls`. The `actions` language has a separate suite that catches `unpinned-tag`, `missing-workflow-permissions`, `untrusted-checkout`, and other CI-specific issues that source-language suites do NOT find. Single-language audits silently miss findings that GitHub's default-setup catches via its `actions` analysis. (2026-04-29: fxhoudinimcp audit ran Python-only; missed `actions/missing-workflow-permissions` in `ci.yml:11`. GitHub default-setup found it.)

3. **Data extensions catch what CodeQL misses.** Even projects using standard frameworks (Django, Spring, Express) have custom wrappers around database calls, request parsing, or shell execution. Skipping the create-data-extensions workflow means missing vulnerabilities in project-specific code paths.

4. **Explicit suite references prevent silent query dropping.** Never pass pack names directly to `codeql database analyze` — each pack's `defaultSuiteFile` applies hidden filters that can produce zero results. Always generate a custom `.qls` suite file.

5. **Zero findings needs investigation, not celebration.** Zero results can indicate poor database quality, missing models, wrong query packs, or silent suite filtering. Investigate before reporting clean.

6. **macOS Apple Silicon requires workarounds for compiled languages.** Exit code 137 is `arm64e`/`arm64` mismatch, not a build failure. Try Homebrew arm64 tools or Rosetta before falling back to `build-mode=none`.

7. **Follow workflows step by step.** Once a workflow is selected, execute it step by step without skipping phases. Each phase gates the next — skipping quality assessment or data extensions leads to incomplete analysis.
...
**Example 1: Python repository scan**
User says: "/codeql" in a Python repo
Actions: Auto-detect Python, build CodeQL database, select security-extended suite, run analysis, process SARIF results, present findings ranked by severity.
Result: Findings report with 0-N vulnerabilities, each with CWE mapping, affected file:line, and data flow description.

**Example 2: Custom data extensions**
User says: "add data extensions for our custom auth decorators"
Actions: Identify decorator patterns in the codebase, create CodeQL data extension YAML mapping decorators to sources/sinks/summaries, re-run analysis with extensions.
Result: Updated scan with reduced false negatives for framework-specific patterns.
