#!/usr/bin/env python3
"""Pure, key-free scoring for the gather-claude live-arm efficacy harness.
...
deterministic instrument that:
  ...
  - tests/test_gather_claude_efficacy.py unit-tests on a tiny synthetic
...
# repo root = skills/<skill>/harness -> parents[2]; add _shared so `import stats` works
# regardless of the importer's CWD (run_live inserts HARNESS, tests load by file path).
    ...
    for k in _METRIC_KEYS:
        ...
        if vals:
            out[k] = {
                ...
                "n": len(vals),
                # per-run values retained so attach_ci can compute a paired CI
                # against the baseline arm (Phase B). Additive; legacy readers ignore.
...
def decide_verdict(with_skill: dict, baseline: dict, primary_metric: str,
    ...
    NB: this gather-claude copy adds `true_recall` to the regression set (the
    gather-research copy checked only refutation/fabrication/grounding). Reason:
    gather-claude's measured failure mode is the framework OVER-rejecting genuine
    current features (true_recall regression), so over-rejection must be a
    first-class "fix" trigger. The regression bar is min_delta (not an epsilon)
    so N=3 noise does not false-trigger.
    """
    def mean(d, k):
    ...
    # excludes 0 unfavorable, else BLOCKED ON MEASUREMENT). The legacy delta-threshold
    ...
    # sub-metric regression check: framework makes a value-dimension meaningfully worse
    regressions = []
    for k in ("refutation_recall", "fabrication_resistance", "grounding_precision", "true_recall"):
        wk, bk = mean(with_skill, k), mean(baseline, k)
        if wk is not None and bk is not None and (bk - wk) > min_delta:
