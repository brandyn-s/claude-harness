...
When running or interpreting mutation tests, **REQUIRED READ**:
`references/mutation-verdict-interpretation.md`. The manifest dependency names
the rule but does not load it; this explicit pointer delivers the verdict
contract for every language, including inline-test layouts that cannot be
identified reliably from a path glob.
