...
when_to_use: 'Use when reviewing PRs, commits, or diffs for security issues. Trigger phrases: "diff review", "review this PR", "what changed", "security review of changes". Do NOT use for SCA compliance (the /sca-review prototype is intentionally unavailable), STIG assessment (use /stig-assess), or general scanning (use /semgrep). Performs security-focused differential review with blast radius calculation, test coverage checks, and markdown reports.'
effort: high
